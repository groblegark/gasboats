...
func (m *Model) renderView() string {
	...
	if m.showHelp {
		...
		b.WriteString("\n")
		b.WriteString(helpStyle.Render("j/k: navigate  1-4: select  r: rationale  d: dismiss  enter: confirm  ?: help  q: quit"))
	}
...
func (m *Model) renderDetailPane() string {
	...
	wrappedPrompt := wrapText(d.Prompt, m.width-4)
	...
	for i, opt := range d.Options {
		...
		if opt.Description != "" {
			wrappedDesc := wrapText(opt.Description, m.width-10)
...
func formatContextDisplay(ctx string, maxWidth int) string {
	if ctx == "" {
		return ""
	}
	...
	if err := json.Unmarshal([]byte(ctx), &parsed); err != nil {
		...
		b.WriteString(detailValueStyle.Render(wrapText(ctx, maxWidth)))
		return b.String()
	}
	...
	if obj, ok := parsed.(map[string]any); ok {
		if schemas, hasSchemas := obj["successor_schemas"]; hasSchemas {
			...
			if schemaMap, ok := schemas.(map[string]any); ok {
...
func colorizeJSONLine(line string) string {
	...
	return jsonValueStyle.Render(line)
}
