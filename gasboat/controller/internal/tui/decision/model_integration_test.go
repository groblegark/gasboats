...
func TestModelUpdateWithFetchDecisionsMsg(t *testing.T) {
	...
	decisions := []DecisionItem{
		{ID: "bd-1", Prompt: "First decision", Urgency: "high", Priority: 1, RequestedAt: now},
		{ID: "bd-2", Prompt: "Second decision", Urgency: "medium", Priority: 2, RequestedAt: now.Add(-1 * time.Hour)},
	}
	...
	// Should be sorted by priority (lower = more urgent)
...
func TestModelKeyNavigation(t *testing.T) {
	...
	m.decisions = []DecisionItem{
		{ID: "1", Urgency: "high", Priority: 1, RequestedAt: now},
		{ID: "2", Urgency: "high", Priority: 1, RequestedAt: now.Add(-1 * time.Hour)},
	...
	m.decisions = []DecisionItem{
		{
			...
			Options: []DecisionOption{{ID: "a", Label: "A", Description: "Option A"}},
			Urgency: "high",
		},
	}
	view = m.View()
	...
	m.decisions = []DecisionItem{
		{
			...
			Options:     []DecisionOption{{ID: "a", Label: "Fix upstream"}, {ID: "b", Label: "Add retry"}},
			Urgency:     "high",
			Priority:    1,
			RequestedAt: now,
